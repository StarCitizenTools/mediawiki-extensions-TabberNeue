...
An optional second table sets attributes on the tabber element, the same ones
`<tabber>` accepts:
```lua
mw.ext.tabber.render(
    {
...
```
Values may be strings, numbers or booleans; `true` means a bare attribute, so
`wrap = true` matches `<tabber wrap>`. Attributes are sanitized as tag
attributes are, so disallowed ones are dropped. An attribute name that is not a
valid HTML attribute name, or a value that is not a string, number or boolean,
raises a Lua error rather than being ignored.

### Parser function

`{{#tag:tabber}}` accepts the same attributes as named arguments, which is
useful when the tab content itself is generated by a template:
```
{{#tag:tabber|
First Tab Title=
First tab content.
